...
function(x,data,true_f = NULL, plot = c("A","f","true_f"), plot_bin = TRUE,
         line = FALSE, confidence = TRUE, high_deg = NULL, shade_point = 0.5, shade_interval = 0.5, max_A = NULL, min_A = NULL, f_min = NULL, 
  ...
  if ("A" == plot[1]) {
     ...
     if ((!is.null(max_A)) && (!is.null(min_A)))
      ...
      abline(h = 1)
          ...
          if (length(non_zero) <= 0)
             stop("There is no data. Please decrease high_deg")  
          b        <- lm(true_f1[non_zero] ~ 0 + x$f[non_zero])$coefficients[1]
          ...
          ylim <- xlim
          plot(true_f1[non_zero][1], b*x$f[non_zero][1], xlim= xlim, ylim = ylim,
          ...
          if (FALSE == plot_true_degree) {
              points(true_f1[non_zero],b*x$f[non_zero],pch = 20,col = rgb(0,0,0,shade_point),...) 
          }
          else {  
              points(b*x$f[non_zero],true_f1[non_zero],pch ="",...)
              col <- brewer.pal(9,"Greens")
              order <- order(data$increase[non_zero])
              col_seq <- seq(min(data$increase[non_zero]),
                         max(data$increase[non_zero]),9)
              a <- sapply(data$increase[non_zero], function(x)which(col_seq >= x)[1])
              a[is.na(a)] <- 9
              text(b*x$f[non_zero],true_f1[non_zero],data$increase[non_zero],col = col[a],...) 
          }
      }
}
